...
module.exports = function migrateSkills243To248(db) {
  const migrate = db.transaction(() => {
    ...
    let inserted = 0;
    for (const skill of skills.filter((s) => s.week_number >= 243 && s.week_number <= 248)) {
  ...
  if (inserted > 0) console.log(`Skill 243—248 增量迁移：新增 ${inserted} 张`);
  return inserted;
};
