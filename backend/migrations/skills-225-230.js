...
module.exports = function migrateSkills225To230(db) {
  const migrate = db.transaction(() => {
    ...
    let inserted = 0;
    for (const skill of skills.filter((s) => s.week_number >= 225 && s.week_number <= 230)) {
  ...
  if (inserted > 0) console.log(`Skill 225—230 增量迁移：新增 ${inserted} 张`);
  return inserted;
};
