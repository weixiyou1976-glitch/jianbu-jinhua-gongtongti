...
module.exports = function migrateSkills237To242(db) {
  const migrate = db.transaction(() => {
    ...
    let inserted = 0;
    for (const skill of skills.filter((s) => s.week_number >= 237 && s.week_number <= 242)) {
  ...
  if (inserted > 0) console.log(`Skill 237—242 增量迁移：新增 ${inserted} 张`);
  return inserted;
};
