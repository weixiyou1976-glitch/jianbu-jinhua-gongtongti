...
module.exports = function migrateSkills231To236(db) {
  const migrate = db.transaction(() => {
    ...
    let inserted = 0;
    for (const skill of skills.filter((s) => s.week_number >= 231 && s.week_number <= 236)) {
  ...
  if (inserted > 0) console.log(`Skill 231—236 增量迁移：新增 ${inserted} 张`);
  return inserted;
};
