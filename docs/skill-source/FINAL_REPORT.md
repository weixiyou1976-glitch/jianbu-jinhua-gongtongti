...
治理实现完成并验证：**332 张、每张 19 个内容字段、2,439 条 skill_tags，与生产快照逐字段一致，差异 0**。所有生产文案原样保留。治理实现基线 commit `b3bab85c42157add47d719ee7c63ee3b6d6c494a` 已提交并推送至独立分支 `codex/skill-source-of-truth`；**尚未合并 main，尚未部署生产，因此 GitHub main 现在仍不能恢复全部 332 张。**
...
**独立分支 `codex/skill-source-of-truth`：可以恢复这 332 张 Skill 的全部内容字段及标签。** 已实际从空 SQLite 恢复验证。该能力尚未进入 main。恢复用户、策印、原数据库 ID、模块关联等需要其他业务备份，不属于这份内容事实源。
...
- `manifest.json` 的 `export_file_sha256` 是完整 UTF-8 生产导出文件的原始字节哈希，覆盖导出元数据、查询说明、格式化空白、Skill、嵌入标签和孤立标签数组，用于确认审计输入文件的完整性。
- 生产快照与对照报告的 `content_sha256` 是紧凑序列化的只读查询结果 `{skills, skill_tags}` 的哈希；其中 Skill 行包含数据库 `id`、19 个内容字段和 `created_at`，标签是独立查询的 2,439 行，用于确认查询结果集合及排序。
- 二者计算对象不同，不应要求数值一致。前者为 `f31ec921…`，后者为 `7e901022…`；仓库与生产的一致性仍由 19 字段和标签的逐项 compare 判定。
...
1. 本治理版本合并 main 后，GitHub JSON 成为内容唯一维护来源；生产是运行副本。合并前以独立分支 `codex/skill-source-of-truth` 保存和审核该治理版本。
...
- 独立分支已经保存并推送完整治理版本；在该分支合并 main 前，main 的恢复缺口仍然存在。原体验修复工作目录保持原状。
